...
import { industries, industryContent } from './industryData';
import type { Industry, IndustryContent, IndustryId } from './industryData';
...
function SectionIntro() {
  return (
    <div className="text-center">
      <Pill size="sm" className="md:h-6 md:px-4 md:text-[10px]">
        Long Subtitle
      </Pill>
...
function MobileIndustryMenu({
  ...
  return (
    <div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-1 rounded-b-[18px] border-x border-b border-gray-100 bg-white px-2 pb-2 md:hidden">
      ...
      {isOpen && (
        <Dropdown className="absolute left-1/2 top-full mt-1 w-[clamp(148px,42vw,178px)] -translate-x-1/2 overflow-hidden rounded-xl border border-gray-100 bg-white py-1 shadow-[0_14px_32px_rgba(15,23,42,0.12)]">
          {industries.map((industry) => (
            <MobileIndustryOption key={industry.id} industry={industry} onClick={() => onChange(industry.id)} />
          ))}
        </Dropdown>
      )}
    </div>
...
function MobileIndustryOption({ industry, onClick }: { industry: Industry; onClick: () => void }) {
...
function IndustryButton({
  ...
  return (
    ...
    >
      <IconLabel icon={industry.icon} iconClassName="h-3.5 w-3.5 lg:h-4 lg:w-4" className="gap-1.5 lg:gap-2">
        {industry.label}
      </IconLabel>
  ...
  return (
    <div className="hidden md:grid md:grid-cols-[minmax(0,0.76fr)_minmax(0,1.24fr)] md:items-start md:gap-6 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:gap-10 xl:grid-cols-2 xl:gap-[70px]">
      <div className="relative min-h-[475px] min-w-0 pl-7 pt-[62px] lg:pl-9">
        ...
        <div className="mt-[92px] space-y-6 lg:space-y-[32px]" onMouseLeave={() => setIndicatorTop(defaultIndicatorTop)}>
...
function NewBadge({ size = 'md' }: { size?: 'sm' | 'md' }) {
  return (
    <Pill size={size === 'sm' ? 'xs' : 'md'} className={size === 'md' ? 'h-6 px-3 text-[10px]' : undefined}>
      NEW
    </Pill>
  );
}
